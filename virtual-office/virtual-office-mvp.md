...
- **관찰자 구조**: 살은 뼈대 활동을 읽기만 한다(관측은 read-only·단방향). 운영에 개입하지 않는다.
- **[2026-07-07 갱신] 사람 자문 입력 채널 추가**: VO 채팅창(하단) → 어댑터 백엔드 프록시 → `/v1/ra/advisory`. 어댑터가 호출 주체라 뼈대는 VO를 모름(단방향 유지). 목적: 에이전트 수준 점검 + KB 갭 발견. 상세: `docs/specs/advisory-chat-channel-spec.md`.
