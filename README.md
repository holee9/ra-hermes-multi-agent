...
**최신 완료 작업:**
- 🩹 **#135 허위 자기검증 + 방어 마커 오발화 — positive framing 수정 (OPEN 유지, 전체 재-eval 대기)** (2026-07-18, commit `db2ceda`): `daily-growth-runner.py` `build_case_content()` Citation rule 재작성. (1) 모델이 요구되지 않은 "no invented identifiers" 자기검증 문장을 창작하며 동시에 창작 → "정확성은 사람 검토자가 확인, 규제 판단으로 마무리"라는 긍정 서술로 자리 제거. (2) 단일 'verify separately' 마커가 아는 것(Rule/Annex 번호)에도 스탬프(5/15) → 인용을 확립된 참조(사실 서술)/케이스 레벨 식별자(마커)로 구분, 마커를 후자로 국한. 금지 문구 아닌 positive framing(#123 R2 교훈). 사전 점검(원 실패 2케이스): 자기검증 소멸, 마커 3→0회, Rule 10/11/17 사실로 11회 서술. 전체 배치 재-eval은 DoD 검증으로 별도(사람 판단).
