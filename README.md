...
**최신 완료 작업:**
- 🛡️ **#118 근거 없는 구체 식별자(510(k) 번호) fabrication 가드 — 코드+배포 완료, 라이브 검증에서 신규 갭 발견, REOPENED** (2026-07-15, [#118](https://github.com/holee9/ra-hermes-multi-agent/issues/118) REOPENED, commit `ed34a10`): #109와 동일한 프롬프트 가드 패턴을 `hermes-api-server.py build_advisory_context()`(실 advisory 경로) + `daily-growth-runner.py build_case_content()`(growth-case/#113 응답캡처 프롬프트) 양쪽에 적용. **재-eval 검증(원 hallucination 케이스 재현)**: "무근거 확정 인용"이 "명시적으로 표시된 예시"로 전환되는 개선 확인(플레이스홀더 문자열 완전 억제는 모델 습성상 한계). **`/opt` 배포 완료**(백업 `.bak-pre-118`, 재시작). **라이브 검증(동일 질의 2회) 결과 혼재**: 1회차는 `evidence` 필드에 DB에 전혀 없는 가상 번호(K210097, K242521) 인용 — 이번 가드가 `recommended_comment`/`summary`만 겨냥하고 "evidence는 이미 source-path-only"라 가정했던 것이 **틀렸음을 라이브로 확인**. 2회차는 실제 NAS 문서 경로(K252912/K213497, 폴더·파일명에 박힌 실존 번호)를 정확히 인용 + 모범적 헤징. RAG 검색 강도(비결정적)에 따라 결과가 갈리는 것으로 추정. **evidence 필드 전용 가드 추가 여부는 사용자 승인 대기** — issue reopen, DoD 확장.
