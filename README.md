...
**최신 완료 작업:**
- 🧠 **"진짜 RA 전문 agent" (a) 학습 이력 주입 구현 + (b) #105 fix 시도·롤백** (2026-07-07, [#104](https://github.com/holee9/ra-hermes-multi-agent/issues/104)·[#105](https://github.com/holee9/ra-hermes-multi-agent/issues/105)): **(a) REQ-AC-002b 학습 이력 주입(구현 완료)** — `build_advisory_context`에 Honcho `daily_growth_case` 최근 7일 학습 주제 주입(`_fetch_learning_history`, date 기반 직접 probe, fail-safe 50-75ms). 에이전트가 과거 학습 반영 회고/자문 가능(SOUL.md 재진술 한계 해소). "학습 이력 자체는 evidence 아님" 명시. **(b) #105 fix 시도 → 라이브 회귀 발견 → 롤백**: 스킬 제거(`ADVISORY_SKILLS=""`)가 직접 단발 테스트(129.6s)에서는 유효했으나, 라이브 배포 후 raspi5p-style email 자문까지 150-200s 행 회귀 — 스킬을 제거해도 agent가 `hermes-cli`(rg) + MCP filesystem으로 `/mnt/nas-ra`를 재검색하며 루프. (b) 코드는 repo에서 revert(`187a892`), production `/opt/hermes-ra/`는 `.bak` 복원으로 안전 복구(바이트 동일). #105 재OPEN — 근본 fix는 advisory를 직접 LLM 호출(도구/루프 없음)로 전환(Contract C 아키텍처 변경, 별도 작업). pytest 31 passed. (a)는 건전하나 production 미배포. 커밋 `518be4d`→`187a892`.
