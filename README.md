...
**최신 완료 작업:**
- 🚨 **#112 딥싱크 — 라이브 advisory RAG 컬렉션(Qdrant) PII 노출 경로 발견 + 정리** (2026-07-15, [#112](https://github.com/holee9/ra-hermes-multi-agent/issues/112) OPEN 유지, commit `9403f9d`): 앞선 인덱싱 배제 작업 점검 중 `sync_ra_knowledge_to_qdrant.py`(매일 03:18 KST)가 `llm-wiki`만 제외하고 QA이력/리서치로그는 필터 없이 Qdrant `ra_kb_markdown`로 복사 중임을 발견 — 이 컬렉션은 `hermes-api-server.py _run_rag_search()`의 **실제 라이브 advisory 서빙 경로**. 실제 이메일·직원 실명·회사 주소가 담긴 812+620건이 라이브 컬렉션에 존재했으나, advisory 로그 전체+Honcho 세션 메시지 4,789건 검색 결과 **실제 인용 사고는 0건**(잠재 위험이었지 발생한 사고 아님, 과장 방지). **① sync 스크립트 배제 필터 추가**(`EXCLUDED_SOURCE_PATTERNS`, cron이 repo 경로 직접 실행이라 별도 배포 불필요) — 구현 중 `%llm-wiki%` 리터럴과 psycopg2 파라미터 바인딩 충돌 버그 1건 재현+수정(`%%` 이스케이프), `verify-sync-ra-knowledge-to-qdrant.py` 신규 PASS, 실측 8238→6806(812+620 배제). **② Qdrant 기존 1,432건 삭제** — 백업(`~/hermes-backups/qdrant-ra_kb_markdown-qa-research-log-20260715.jsonl`, payload+vector 전체) → filter-delete → 카운트 0 확인(8209→6777) → 실제 advisory 쿼리로 회귀 확인(confidence 0.94, 정상). **③ pgvector 원본 812+620행은 보존**(사용자 선택 — 감사 보존 가치, 실질 노출 경로는 ①②로 이미 차단됨).
- 🔒 **#112 인덱싱 파이프라인 배제 규칙 정의 + 프로덕션 배포 완료** (2026-07-15, [#112](https://github.com/holee9/ra-hermes-multi-agent/issues/112) OPEN 유지, commit `02c7a24`): `scripts/index_github_repos.py`에 `INDEX_EXCLUDED_PATH_PATTERNS`(env 튜닝 가능) + `is_excluded_path()` 추가 — `06_심사_QA이력`/`11_일일_리서치로그`/`wiki/entities` 경로를 fetch·embed·ra_knowledge write **이전 단계**에서 배제(retrieval-side와 달리 애초에 인덱싱 자체를 차단). `list_md_files()`·`list_md_files_gitea()` 양쪽 적용. 검증: `verify-index-github-repos.py` 신규(제외 3종+정상 3종+필터 재현) PASS, lint clean. **`/opt/hermes-ra/index_github_repos.py` 배포**(cron 배치, 백업 `.bak-pre-112` 보존, 서비스 재시작 불필요) + **라이브 실행 검증**: 마침 MD-process 신규 커밋(AUDIT 문서 6건)이 있어 실동작 확인 — 배제 패턴 신규 유입 0건(QA이력 812/리서치로그 620 chunks 변화 없음), 전체 rows 8209→8238(+29, 신규분과 정확히 일치). DoD 잔여 1항목("기존 PII 청크 격리·제거")은 감사 보존 원칙상 별도 사람 승인 필요 — OPEN 유지.
- 🛠️ **#111 README 목차(manifest) 청크 필터 보강 — 재-eval로 개선 실측 후 CLOSED** (2026-07-15, [#111](https://github.com/holee9/ra-hermes-multi-agent/issues/111) CLOSED, commit `fb6a41c`): 이전 재-eval에서 식별된 갭(하이퍼링크 없는 불릿/번호 목차가 `is_substantive_chunk()` 필터를 통과) 수정 — heading 제외 라인 중 목록형이 60% 이상이면서 80자 넘는 실질 문장이 없으면 비실질로 판정하는 3번째 조건 추가. 회귀 테스트 3종(목차 불릿 reject·목차 번호 reject·실질 분석 불릿 유지) 추가, `verify-daily-growth-runner.py` PASS. **재-eval 실측(iteration 10-12 vs 07-09, 프로덕션 `assemble_cases`)**: README-sourced TOC성 excerpt **13.3%(6/45) → 4.4%(2/45)**. 직접 확인: `NB_심사자료/README.md`가 1번째 청크(TOC) 대신 2번째 청크(실질 NB 인증 내용)로 정확히 전환. 잔여 2건은 필터 결함이 아니라 `MDR_2017_745/README.md` 자체가 인덱싱된 청크 3개 전부 목차성이라 대체할 실질 청크가 없는 문서-콘텐츠 한계(fallback이 케이스를 굶기지 않은 것).
- 🔎 **#110~112 KB retrieval 품질 재-eval 실행** (2026-07-15): PR #114(84ffed5) 반영 후 신규 checksheet 45케이스(iteration 07-09, `docs/kb-eval-checksheets/2026-07-15/`, 프로덕션과 동일한 `assemble_cases` 로직)로 AI 기반 재검증. **[#110](https://github.com/holee9/ra-hermes-multi-agent/issues/110) CLOSED**: 광의 키워드(FDA 단독) 오매칭 직접 재현 테스트 — focus=`510(k) predicate strategy`로 FDA 매칭 시 이전엔 cybersecurity/threat-model 문서였으나 이번엔 510(k)/RTA 계열로 정합, 전체 mismatch 3/45(6.7%, 전부 경계 케이스). **#112 OPEN 유지(retrieval-side만 충족)**: QA이력/리서치로그 패턴 소스 0건 확인(retrieval 가드 정상)이나, 근본 대응(인덱싱 파이프라인 배제 + 기존 PII 청크 격리)은 사람 승인 게이트 필요로 범위 밖.
- 🚀 **#109 RA Advisory 프롬프트 — summary 본문분석 강제 프로덕션 배포** (2026-07-15, [#109](https://github.com/holee9/ra-hermes-multi-agent/issues/109) CLOSED): 2026-07-11 커밋(`16cdf91`, summary는 `Subject:` 라인 복사/변형 금지 + 본문 분석 기반 + 100자 이내 가드)이 `/opt` 배포본과 7줄 차이로 미반영 상태였던 것을 사용자 승인(GATE-3) 받아 동기화. `/opt/hermes-ra/hermes-api-server.py` repo와 byte-identical 배포(백업 `.bak-pre-109` 보존) + `hermes-api-server.service` 재시작(active). **라이브 검증**: `Subject: 510k premarket notification deadline question`을 본문 앞에 삽입한 유도 쿼리로 실 advisory 호출 → 응답 `summary`가 Subject 영문을 복사하지 않고 본문(한국어 규제 질의) 기반 85자 요약으로 정상 생성 확인.
