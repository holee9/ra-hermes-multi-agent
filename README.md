...
**최신 완료 작업:**
- 🩹 **#129 ra_kr 구체 규정번호 창작 — 식별자 원칙 일반화 positive framing 수정 (OPEN, 전체 재-eval 대기)** (2026-07-19, commit `9f...`): #124가 OECD/CER 문장에만 국한돼 모델이 다른 주제에서 한국 고시번호 계속 발명(2022-123/2021-06/2025-02/제23조 — 소스 대조로 전부 모델 발명 확인). 공유 Citation rule(`build_case_content`, 전 에이전트)의 case(2) "instance identifiers"를 device 번호 + 구체 dated 규정번호(고시/notice/decree) + 특정 조/항/호까지 확장 + ra-kr SOUL.md Fixed Rule 6(구체 규정번호는 소스에서, 발명 금지) + OECD MAD(비임상 GLP) vs 임상 CER 구분. 사전 점검(원 실패 3케이스): 4개 발명 식별자 전부 소멸, 헤징/말 서술로 대체. positive framing(#123 R2 교훈). daily-growth는 다음 배치 자동 반영, 라이브는 서비스 재시작 대기.
