...
**최신 완료 작업:**
- 🛡️ **#118 근거 없는 구체 식별자 fabrication — 검증+로깅+정규식 버그 수정 완료, CLOSED** (2026-07-16, [#118](https://github.com/holee9/ra-hermes-multi-agent/issues/118) CLOSED, commit `d3db662`+`2f88408`): evidence 필드까지 커버하는 Part A(프롬프트)+Part B(결정론적 사후검증, `validate_advisory()`에 `_shown_source_text()`+`_cited_identifier_status()` 추가, 신규 `yellow_reason="unverified_identifier"`) 구현 후 `/opt` 배포(백업 `.bak-pre-118b`, 재시작). **배포 후 검증**: glass-box 직접호출 11회 전부 정상, black-box curl 1건은 사후 확정 불가(관측 공백)로 판명 → **후속 조치**: `_log_adv_request`에 `cited_identifier_status`(식별자별 검증 여부) 기록 추가해 향후 유사 분쟁을 사후 확정 진단 가능하게 함. **부수 발견(테스트 작성 중)**: `\bK\d{6}\b` 정규식이 "K222222도 확인"처럼 **한글 조사가 공백 없이 붙는 자연스러운 한국어 문장에서 매칭 실패**(Python `re`의 `\b`가 유니코드 모드에서 한글을 단어문자로 취급) — 이 상태론 해당 식별자가 추출조차 안 돼 검증을 조용히 우회하는 가장 위험한 실패 형태였음. negative lookaround로 경계 재정의해 수정. **검증**: `tests/test_advisory.py` 신규 9종(Part B 5종+로깅/정규식 4종) 포함 44 passed. `/opt` 재배포는 별도 결정.
