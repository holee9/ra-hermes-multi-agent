...
**최신 완료 작업:**
- 🛡️ **#118 근거 없는 구체 식별자 fabrication — evidence 필드까지 확장(Part A+B) + /opt 배포 완료, CLOSED** (2026-07-16, [#118](https://github.com/holee9/ra-hermes-multi-agent/issues/118) CLOSED, commit `d3db662`): 라이브 검증에서 드러난 갭(evidence 배열에도 무근거 식별자 삽입)을 딥싱크 후 2단계로 해결. **Part A(프롬프트)**: evidence 배열은 제시된 경로 그대로만 사용, 부가 설명 금지. **Part B(결정론적 사후검증)**: `validate_advisory()`에 `_shown_source_text()`(RAG+wiki_results 전체를 LLM이 실제로 본 텍스트로 재구성) + `_unverified_identifiers()`(510(k)/PMA 식별자 추출 후 shown text 존재 여부 대조, 공백·하이픈 무시) 추가 — 없으면 신규 `yellow_reason="unverified_identifier"`. 하위호환, 추론 로직 미접촉(#105 원칙). **검증**: `tests/test_advisory.py` 신규 5종 포함 40 passed. **`/opt` 배포 완료**(백업 `.bak-pre-118b`, 재시작). **배포 후 검증**: glass-box 직접호출 11회(사전 6회+사후 5회) 전부 정상(실제 문서 번호만 인용, unverified 0건). black-box curl 2회 중 1회는 evidence에 `K210097,K242521`이 안 걸리고 통과 — 격리 재현 시 로직은 정상 detect(11/11 정상)하지만, `_log_adv_request`가 호출 시점의 shown_source_text를 기록하지 않아 그 1건은 사후 포렌식으로 확정 불가(버그 아닌 **관측 공백**으로 명시) — 로그에 shown_source_text 추가는 권장 후속(별도 승인 시).
