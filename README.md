...
**최신 완료 작업:**
- 🛡️ **#118 근거 없는 구체 식별자 fabrication — evidence 필드까지 확장 완료(Part A+B), CLOSED** (2026-07-16, [#118](https://github.com/holee9/ra-hermes-multi-agent/issues/118) CLOSED, commit `d3db662`): 라이브 검증에서 드러난 갭(evidence 배열에도 무근거 식별자 삽입, recommended_comment/summary 가드는 미커버)을 딥싱크 후 2단계로 해결. **Part A(프롬프트)**: "evidence 배열은 제시된 경로를 그대로만 사용, 부가 설명/식별자 금지" 지시 추가. **Part B(결정론적 사후검증, 근본 해결)**: `validate_advisory()`(기존 안전판 함수, 추론 로직 미접촉 — #105 원칙 부합)에 새 체크 추가 — `_shown_source_text()`로 RAG+wiki_results(llm_wiki/openfda/law_kr) 전체를 LLM이 실제로 본 텍스트로 재구성, `_unverified_identifiers()`로 advisory 출력에서 510(k)/PMA 식별자를 추출해 공백·하이픈 무시하고 shown text에 실존하는지 대조 — 없으면 신규 `yellow_reason="unverified_identifier"`로 강등. 하위호환 파라미터로 기존 호출부 무회귀. **검증**: `tests/test_advisory.py` 신규 5종 포함 40 passed. **라이브 재현 6회**(원 실패 질의 3회+약한 RAG 유도 질의 3회) — Part A만으로 0/6 fabrication(개선 전 ~50%에서 대폭 감소), Part B는 유닛 테스트로 로직 정확성 확인. `/opt` 배포는 별도 결정.
