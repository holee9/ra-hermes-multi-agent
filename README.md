...
**최신 완료 작업:**
- 🔒 **#112 인덱싱 파이프라인 배제 규칙 정의** (2026-07-15, [#112](https://github.com/holee9/ra-hermes-multi-agent/issues/112) OPEN 유지, commit `02c7a24`): `scripts/index_github_repos.py`에 `INDEX_EXCLUDED_PATH_PATTERNS`(env 튜닝 가능) + `is_excluded_path()` 추가 — `06_심사_QA이력`/`11_일일_리서치로그`/`wiki/entities` 경로를 fetch·embed·ra_knowledge write **이전 단계**에서 배제(retrieval-side와 달리 애초에 인덱싱 자체를 차단). `list_md_files()`·`list_md_files_gitea()` 양쪽 적용. 검증: `verify-index-github-repos.py` 신규(제외 3종+정상 3종+필터 재현) PASS, lint clean. **DB 실측**: 기존 인덱싱분 812 chunks(QA이력 14 sources)+620 chunks(리서치로그 75 sources)는 **미변경**(이번 변경은 향후 재인덱싱만 차단). DoD 잔여 1항목("기존 PII 청크 격리·제거")은 감사 보존 원칙상 별도 사람 승인 필요 — OPEN 유지. `/opt` 배포도 별도 승인 시 진행.
