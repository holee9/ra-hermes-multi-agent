...
**최신 완료 작업:**
- 🛡️ **#118 근거 없는 구체 식별자(510(k) 번호) fabrication 가드 추가, CLOSED** (2026-07-15, [#118](https://github.com/holee9/ra-hermes-multi-agent/issues/118) CLOSED, commit `ed34a10`): #109와 동일한 프롬프트 가드 패턴을 `hermes-api-server.py build_advisory_context()`(실 advisory 경로) + `daily-growth-runner.py build_case_content()`(growth-case/#113 응답캡처 프롬프트) 양쪽에 적용 — "구체 식별자는 source에 실제 있을 때만 인용, 없으면 예시 번호도 제시하지 말고 '문서 확인 필요'라고 명시". **재-eval 검증(live GX10 재현 테스트, 원 hallucination 케이스 3종 재실행)**: 2케이스는 가상 번호 완전 미출현, 1케이스는 5회 중 3회 여전히 `K123456` 재현되나 **재현될 때마다 예외 없이 "example only — verify separately"류 면책 문구가 동반** — 가드 전(원본 iteration-16)엔 이런 표시 없이 실제 인용처럼 제시됐던 것과 대조적. 결론: "무근거 확정 인용"을 "명시적으로 표시된 예시"로 전환하는 데는 성공, 플레이스홀더 문자열 자체의 완전 억제는 모델 습성상 프롬프트만으로는 확률적 한계 있음(추가 원할 시 출력 후처리 레이어 필요). `/opt` 배포는 별도 결정.
