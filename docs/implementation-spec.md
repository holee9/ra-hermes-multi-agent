...
- 정적 대시보드: `scripts/render-growth-dashboard.py`가 로컬 reports와 systemd 상태를 요약해 `docs/growth-dashboard.html`을 생성한다. HTML은 외부 fetch 없이 동작하는 standalone snapshot이며 `scripts/verify-growth-dashboard.py`로 계약을 검증한다.
