...
- Diagnostic/current report: `reports/growth-diagnostic-2026-06-16.json` 기준 `sessions_listed=32`, `sessions_with_messages=27`, `messages_scanned=302`, `empty_cause=metrics_input_available`; `reports/growth-2026-06-19.json` 기준 `sessions_scanned=27`, `messages_scanned=302`, 행동/사람 피드백 metric은 아직 N/A 또는 denominator 0.
- Growth Trend Verdict는 messages/sessions 입력만으로 `측정 가능` 처리하지 않는다. phase2 행동 지표와 feedback coverage 값이 실제 관측될 때까지 `측정 불충분` warning으로 유지한다.
