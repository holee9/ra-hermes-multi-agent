...
`reports/growth-YYYY-MM-DD.json` 은 `.gitignore` 대상이라 **수집 호스트(T3610) 로컬 산출물**이다. 소비 호스트
(raspi5p 등)에서 `growth-transition-readiness.py` 를 그냥 돌리면 리포트가 0건이라 게이트가 "blocked" 로 보인다 —
성장 정체가 아니라 호스트 불일치다(#103). 스크립트는 이 경우 `no_reports_hint` 를 결과에 넣는다.

전달은 **기존 API 인증·네트워크 경로**를 재사용한다(새 SSH 키·자격증명 없음). T3610 이 읽기 전용으로 내보내고,
소비 호스트가 없는 날짜만 받아 채운다. 쓰기 엔드포인트는 없다 — 리포트는 T3610 이 단독 생성한다.

| 쪽 | 수단 |
|---|---|
| T3610 (제공) | `GET /v1/growth/reports` (날짜 목록), `GET /v1/growth/reports/<YYYY-MM-DD>` (원문). 둘 다 `Authorization: Bearer $API_SERVER_KEY` 필수. symlink·경로 이탈 파일은 서빙하지 않음 |
| 소비 호스트 (수신) | `API_BASE=http://<t3610>:8643 API_SERVER_KEY=... python3 scripts/fetch-growth-reports.py --days 30` — 없는 날짜만 원자적으로 받아 로컬 `reports/` 에 채운다. `--dry-run` 으로 목록만 확인. 종료코드 0 정상 / 1 일부 실패 / 2 연결·인증 실패 |
...
- readiness matrix는 15/16이다. 남은 1점은 `ra_kr >= ra_eu 20%` legacy relative depth floor이며, timer는 명시 승인 전까지 OFF를 유지한다. 이 20% floor는 KR corpus가 거의 빈 상태로 남지 않게 막는 pre-activation guard이고 전문가 성숙도 기준이 아니다.
...
- `ra_eu=2956` 기준 `ra_kr >= int(ra_eu * 0.2)` legacy pre-activation floor를 통과했다.
