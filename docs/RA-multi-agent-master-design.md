...
> 원 설계·검증 기록 기준일: 2026-06-21. 장비 역할·관측 모델 부분 개정: 2026-09-09 (라이브 재검증 아님). 현재 운영 판정은 [운영 전략 §0](operations-guide.md#0-현재-운영-판정-2026-09-09) 참조.
...
| **GX10** | GB10 Grace Blackwell(ARM), 로컬 LLM | 추론·임베딩 백엔드 / 영상처리·코딩 / 인프라 에이전트 |
| **라즈베리파이 5+** | RAM 16G | OpenProject / n8n / 업무 담당·n8n 담당·인프라 에이전트 |

- GX10에 추론·임베딩을 위임하는 구성이 기본이다. RA 자문과 프로파일 생성의 코드 기본 모델은 `gpt-oss:120b`, 임베딩은 `qwen3-embedding:latest`이며, Honcho 추론 모델은 별도 환경 설정이다. GLM 등 endpoint 전환 가능성을 포함하므로 모든 경로가 Qwen3를 쓴다고 가정하지 않는다. 초기 Qwen3 연결 검증 이력은 아래에 보존한다.
- n8n은 RPi 배치 기준이다. 2026-06-19 T3610 이전 선언은 2026-06-23 철회됐다. 이후 T3610에 RA Advisory API가 추가됐으며, 자문과 실행 권한을 분리한다. 현재 가동 상태는 배포 환경에서 별도 확인한다.
...
- **관찰자 구조**: 활동 기록을 읽어 관측한다. 과거는 정적 이력, 새 활동만 실시간 애니메이션으로 표현한다(2026-06-21 #74 전환). 과거 재생이나 캐릭터 동작을 새로운 업무 수행으로 해석하지 않는다.
- **목업 = 계약**: 뼈대 출력 형태(conclusion·투표·매칭·3점평가)를 목업으로 고정 → 살을 독립 선개발 → 연동 시 `DATA_SOURCE`만 목업→Honcho 교체.
- 뼈대는 시각화 존재를 모르고 운영 기록만 남김(단방향).
- **[2026-07-07 갱신]** 관측은 read-only 단방향 유지. 사람→RA 자문 입력 채널(VO 채팅 → 어댑터 백엔드 프록시 → `/v1/ra/advisory`)이 별도 추가 — 어댑터가 API 호출 주체라 뼈대(Hermes)는 여전히 VO를 모름. 목적: 에이전트 수준 점검 + KB 갭 발견. 상세: `docs/specs/advisory-chat-channel-spec.md`.
- Honcho 기록을 읽는 `virtual-office/virtual-office-honcho-adapter.js`가 구현되어 있다. 학습량 별·coverage·KB 갭은 관측 지표이며, 정확도 축은 코드상 `pending`이다. [성숙도 명세](specs/maturity-capability-spec.md) 참조.
...
| 가상 오피스 | 활동 시각화 + 사람 자문 입력(어댑터 프록시 경유, 뼈대는 VO 모름) | 본 시스템 활동 기록 단방향 읽기 + 자문 요청(어댑터가 `/v1/ra/advisory` 호출) |
