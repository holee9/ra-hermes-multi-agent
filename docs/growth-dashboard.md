...
python3 scripts/check-growth-metrics-health.py   # 매일 회귀 감지 (API 실패/데이터 품질 점검)
```
...
2026-06-20 snapshot도 `sessions_scanned=0`, `messages_scanned=0`으로 회귀했다. 이번엔 #64 보정과 무관하게 timer 실행 시점(KST 02:00)에 Honcho가 `POST /v3/workspaces/work/sessions/list`에서 500 Internal Server Error를 반환한 것이다(오전 장애, 현재 복구). 이런 일시적 API 장애를 즉시 감지하기 위해 `scripts/check-growth-metrics-health.py`를 매일 점검 루틴에 추가했다. 이 과정에서 같은 스크립트의 `timedelta.hours` 속성 오류(`AttributeError`, 스크립트 자체가 미동작)와 `render-growth-dashboard.py`의 `systemctl` bare 호출(PATH 미탑재 환경에서 timer 상태 누락) 두 결함을 함께 수정했다.
