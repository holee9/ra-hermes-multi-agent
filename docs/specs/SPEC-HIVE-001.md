...
| **P3 최소 런타임 연결** | `ra_us`·`ra_eu` 2개 peer, 수락/처리 신호, drain gate, hop guard, 수동 pause/resume. 하위: P3-0 Hermes idle/accept 실측(delivery-gate §3.1 조사 지점) → P3-1 상태 소스 계약(§3.1, 완료) → P3-2 라우터 gate 훅+회귀 → P3-3 파일럿 예산 | 사람 승인한 메시지 예산·실행 시간·복구 절차 | P3-1 문서·P3-2 판정 참조 구현 완료, P3-0 실측 대기 |
