...
| P3-0 결정안 — HIVE actor/conversation ↔ Hermes session (**사람 승인 대기**) | 제안: **상주 세션을 만들지 않는다.** HIVE 메시지 1건 = `hermes -p <profile>` 일회성 호출 1회, conversation 문맥은 라우터 log의 스레드에서 `-z context`로 재구성해 넘긴다(일회성 advisory를 임의 상주 세션으로 합치지 않음). 직렬화는 **유일 호출자인 API 서버**가 profile별 lock으로 수행하고, 그 lock 보유 = 그 actor의 authoritative `busy`, 미보유 = `idle`. `accepted` = lock 획득 후 subprocess 기동, `handled` = subprocess 정상 종료 + outbox 답신 기록. 이 결정은 세션 모델·비용(호출마다 문맥 재투입)에 영향하므로 구현 전 사람 승인 항목이며, 승인 전에는 sink를 붙이지 않는다 |
