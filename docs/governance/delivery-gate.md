...
| API 구현 (P3-0 (2)~(4), 세션 모델 A) | `GET /v1/hive/state/<actor>`: `idle/busy` + tz 시각 + `generation`(프로세스 세대) + `scope: this-api-process-only`. `POST /v1/hive/submit`: 같은 lock 안에서 "profile 미점유 → 수락+점유 → 원장 `accepted` 영속" 뒤 subprocess 실행, 점유 중이면 `409 reject-busy`, 같은 `msg_id` 재제출은 원장 상태 반환(실행 없음). `GET /v1/hive/lookup/<msg_id>`: drain 재시작 lookup. 기존 chat-completions·헬퍼도 같은 점유 카운터를 올려 state에 반영(동작 변경 없음). 원장은 `HIVE_LEDGER_PATH` jsonl(미설정 시 메모리 → 재시작 후 lookup은 404=unknown 취급). 문맥은 payload만(스레드 재구성은 (5) 실측 후). `completed`≠`handled` |
