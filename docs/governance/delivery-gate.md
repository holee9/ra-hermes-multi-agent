...
| P3-0 추가 근거 (codex, 코드 읽기만) | Hermes CLI(`cli.py` 3399 부근)는 resume 없이 호출마다 **새 session id**를 만들고, 현 RA argv에도 resume이 없다. `active_sessions` lease는 **총 개수 상한**이지 actor/profile별 직렬화가 아니다(cap 비활성 시 noop, 예외 시 CLI 진행). 즉 같은 profile의 동시 2건을 막는 장치가 Hermes 쪽에 없다 |
| P3-0 결정안 — HIVE actor/conversation ↔ Hermes session (**사람 승인 대기**) | 제안: **상주 세션을 만들지 않는다.** HIVE 메시지 1건 = `hermes -p <profile>` 일회성 호출 1회, conversation 문맥은 라우터 log의 스레드에서 `-z context`로 재구성(G3 재사용 대안 — Hermes `resume`으로 스레드를 잇는 안 — 과 비용·격리 비교 후 확정). 직렬화는 API 서버의 profile별 lock이 맡되, **범위 한정**: (a) "repo 유일 호출자"는 host 유일과 다르다 — lock은 `_invoke_hermes`와 `/v1/chat/completions` 두 경로를 모두 포괄해야 하고, host의 다른 `hermes` 호출은 커버하지 못하므로 lock 미보유는 "이 API 프로세스가 관측한 범위에서 idle"로만 읽는다; (b) 재시작 generation(프로세스 시작 시각/카운터)을 상태에 실어 죽은 프로세스의 lock 잔재를 stale로 판정; (c) `accepted`는 spawn이 아니라 subprocess가 입력을 읽어 처리 개시를 기록한 시점, `handled`는 정상 exit이 아니라 corr 답신·의무(§5.1) 검증 완료. "Hermes에 profile별 직렬화 장치가 없다"는 단정은 확인한 `active_sessions` lease 경로에 한정한다. 승인 전에는 sink를 붙이지 않는다 |
| 제출 내구성(P3-2, codex 재현) | dry-run(기본)은 어떤 파일도 쓰지 않고 sink를 부르지 않는다. execute는 sink 호출 **전에** `submitting`을 저널에 영속(intent-before-submit); sink 뒤 저장 실패·중단 후 재시작은 sink를 다시 부르지 않고 수신측 **안정 id 조회**(lookup)로만 결과를 확정한다 — 조회 수단이 없으면 `ambiguous-submit` 보류(사람 확인). 수신측은 HIVE `id`로 멱등해야 하며, 이것이 P3-0 수락 계약의 인수 항목이다 |
